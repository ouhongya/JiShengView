	...
	<view class="statistics">
		<u-sticky :offset-top="offset">
			<view class="condition">
				<view class="search-part">
					<view class="limit">
						<u-input v-model="fromlist.list1" type="select" :border="true" @click="openlist1()" height="70" placeholder="点击选择单位"/>
						<!--选择单位弹框 -->
				...
				<view class="search-part">
					<view class="limit">
						<u-input v-model="fromlist.list2" type="select" :border="true" @click="openlist2()" height="70" placeholder="点击选择人员"/>
						<!--选择人员弹框 -->
				...
				<view class="search-part">
					<view class="limit">
						<u-input v-model="fromlist.list3" type="select" :border="true" @click="openlist3()" height="70" placeholder='点击选择准则'/>
						<!--选择准则弹框 -->
		...
		<view class="table-part">
			<view class="part">
				...
				<view class="table-content1" v-else>
			...
			<view class="part">
				...
				<view class="table-content">
					...
					<scroll-view scroll-y style="height:400rpx;" v-else>
						<u-table>
							<u-tr v-for="item in TaskOverDetail">
								<u-td width="20%">{{item.unitName?item.unitName:'暂无'}}</u-td>
								<u-td width="20%">{{item.name?item.name:'暂无'}}</u-td>
								<u-td width="20%">{{item.censor?item.censor:'0'}}/{{item.total_censor?item.total_censor:'0'}}</u-td>
								<u-td width="20%">
									<view v-if="functionid==2">
										...
										<view v-else>
											...
											<view class="state blue" v-if="item.status==8">
												待审核
											</view>
											<view class="state blue" v-if="item.status==9">
												待分配
											</view>
											<view class="state green" v-if="item.status==10">
												已归档
											</view>
											<view class="state blue" v-if="item.status==11">
												整改中
											</view>
										</view>
									</view>
									<view v-if="functionid==3">
										<view v-if="item.type==2">
											...
											<view class="state green" v-if="item.status==9">
												已完结
											</view>
											<view class="state yellow" v-if="item.status==10">
												审核驳回
								...
								<u-td>{{item.total_issue?item.total_issue:'0'}}</u-td>
							</u-tr>
						</u-table>
			...
			<view class="part">
				...
				<view class="table-content" v-else>
					<canvas canvas-id="table2" id="table2" class="charts" disable-scroll=true @touchstart="touchTable2" @touchmove="moveTable2"
					 @touchend="touchEndTable2"></canvas>
				</view>
			</view>
			<view class="part">
				<view class="table-title">
				...
				<view class="empty" v-if="table3Flag">
					<image src="../../static/nodata.png" mode=""></image>
					<view class="empty-tips">
						暂无数据
					</view>
				...
				<view class="table-content" v-else>
					<canvas canvas-id="table3" id="table3" class="charts2" @touchstart="touchTable3"></canvas>
				</view>
			</view>
			<view class="part">
				<view class="table-title">
				...
				<view class="table-content" v-else>
					<canvas canvas-id="table4" id="table4" class="charts" disable-scroll=true @touchstart="touchTable4" @touchmove="moveTable4"
					 @touchend="touchEndTable4"></canvas>
				</view>
			</view>
			<view class="part">
				<view class="table-title">
				...
				<view class="table-content" v-else>
					<canvas canvas-id="table5" id="table5" class="charts" disable-scroll=true @touchstart="touchTable5" @touchmove="moveTable5"
					 @touchend="touchEndTable5"></canvas>
				</view>
			</view>
					...
					<scroll-view scroll-y style="height:400rpx;" v-else>
						<u-table>
							<u-tr v-for="item in workload">
								<u-td width="20%">{{item.name?item.name:'暂无'}}</u-td>
								<u-td width="20%">{{item.checked?item.checked:'0'}}</u-td>
								<u-td width="20%">{{item.total_issue?item.total_issue:'0'}}</u-td>
								<u-td width="20%">{{item.solved_issue?item.solved_issue:"0"}}</u-td>
...
<script>
	...
	export default {
		...
		mounted() {
			...
			// 	windowHeight
			// } = uni.getSystemInfoSync();
			// let canvasW = (windowWidth - 30) * 2
			_self = this;
			this.uid = uni.getStorageSync("USER_ID")
			//圆弧进度图 
			this.cWidth1 = uni.upx2px(400);
		...
		methods: {
			...
			queryTaskOverDetail(data) {
				helper.request(helper.websiteUrl + 'v1/queryTaskOverDetail', data, 'POST', true, res => {
					if (res != null) {
						this.TaskOverDetail.forEach(item=>{
							item.functionId=this.functionid;
						})
			...
			//拉去统计图的列表
			queryStatisticsList(unitId, userId, excelId) {
				let param = {
					unitId: unitId,
					userId: userId,
					excelId: excelId,
				...
				helper.request(helper.websiteUrl + 'v1/queryStatisticsList', data, 'POST', true, res => {
					if (res != null) {
						...
						if (res.speed) {
							...
							if (table1 != null) {
								console.log('1111table1')
								table1.updateData({
									series:this.arcbar.series,
									animation:false
								});
							} else {
								console.log('2222table1')
								this.getRingData();
							}
						} else {
						...
						if (res.siteList&&res.siteList.length != 0) {
							...
							if (table2 != null) {
								// table2.updateData(this.Unit.series)
								// table2.updateData(this.Unit.categories)
								console.log('1111table2')
								table2.updateData({
							...
							if (table3 != null) {
								// table3.updateData(this.Pie.series)
								console.log('1111table3')
								table3.updateData({
									series: this.Pie.series,
								});
							} else {
								console.log('2222table3')
								this.getPieData();
							}

						} else {
							this.table2Flag = true;
							this.table3Flag = true;
						}


						//检查模块问题统计
						if (res.issueModule&&res.issueModule.length != 0) {
							let arr = []
							let array = []
							...
							if (table4 != null) {
								// table4.updateData(this.Check.series)
								// table4.updateData(this.Check.categories)
								console.log('111table4')
								table4.updateData({
						...
						//相同问题数量统计图
						if (res.issueCountIds&&res.issueCountIds.length != 0) {
							let arr = []
							let obj = []
							...
							if (table5 != null) {
								// table5.updateData(this.Similar.series)
								// table5.updateData(this.Similar.categories)
								console.log('111table5')
								table5.updateData({
			...
			getRingData() {
				table1 = new uCharts({
					...
					series: this.arcbar.series, //这里是数据
					animation: true,
					width: this.cWidth1*this.pixelRatio,
					height: this.cHeight1*this.pixelRatio,
					dataLabel: true,
					title: {
						name: Math.round(this.arcbar.series[0].data*100)+'%',
						color: this.arcbar.series[0].color,
						fontSize: 25
					},
					subtitle: {
						name: this.arcbar.series[0].name,
						color: '#666666',
						fontSize: 15
					},
					extra: {
				...
				if (table3 != null) {
					table3.showToolTip(e, {
						format: function(item) {
							return item.name + ':' + '问题数量' + item.data
						}
					});
				}
			...
			// 这里是筛选部分 (请求接口时调用close1,close2,close3方法)
			//选择单位
			checkboxChange(e) {},
			...
			close1() {
				...
				this.queryStatisticsList(list1, list2, list3)
			},
			//选择人员
			checkboxChange2(e) {},
