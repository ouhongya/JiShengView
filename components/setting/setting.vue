				...
				<scroll-view scroll-y style="height:70vh;" @scrolltolower="onreachBottom3">
					<view class="u-collapse-item" v-for="(item,index) in collapseList">
						...
						<view class="u-collapse-body" v-if="item.isShow">
							...
							<view class="u-collapse-content" v-for="(i,index1) in item.secondunit">
			...
			<u-popup v-model="showFields" :mode="curMode" width="40%" height="80%" @close='closeFields'>
				<view class="roleModel">
					...
					<view class="addcount" v-if="FieldsType">
						...
						<u-icon @click="deleteFields()" name="minus" color="#fa3534" size="40" class="personicon"></u-icon>
					</view>
	...
	export default {
		...
		methods: {
			...
			searchFields() {
				...
				this.doneLoading = true;
				this.statusz = 'nomore';
				if (this.fieldsName == '' || this.fieldsName == null) {
					this.initz();
				} else {
					helper.request(helper.websiteUrl + "v1/unitlistsearch", {
						'uid': '',
						"NAME": this.fieldsName
					}, 'GET', false, (data) => {
						this.collapseList = this.collapseList.concat(data);
			...
			openSearch(item, index) {
				...
				helper.request(helper.websiteUrl + "v1/unitsecondlist", {
					...
					this.collapseList[index].secondunit = data;
					this.collapseList[index].isShow = true;
				})
			},
			clearFields() {
				this.pageNumz = 1;
			...
			handleChild(index,type,index1,i) {				
				this.handleunitindex = index;				
				this.parentcompany = this.collapseList[index].NAME;
				this.parentcompanyid = this.collapseList[index].UNIT_ID;
				if (type == 1) {
					this.ChildList = [{
						company: '',
						name: '',
					...
					this.handleunitindex1 = index1;
					this.bianunitid = i.UNIT_ID;
					this.ChildList = [{
						company: i.NAME,
						name: i.GROUPUSER,
						phone: i.PHONE,
			...
			sureChild(type) {
				...
				if (type == true) {
					...
					helper.request(helper.websiteUrl + "v1/saveunitsecond", {
						...
						helper.showToast("新增成功！")
						this.collapseList[this.handleunitindex].isShow = false;
						this.collapseList[this.handleunitindex].secondunit = data;
						this.collapseList[this.handleunitindex].isShow = true;
					})
					this.showChild = false;
				} else {
					//编辑	
			...
			Childel() {
				...
				helper.request(helper.websiteUrl + "v1/deleteunit", {
					...
					helper.showToast(data)
					this.collapseList[this.deleteunitindex].isShow = false;
					this.collapseList[this.deleteunitindex].secondunit.splice(this.deleteunitindex1, 1)
					this.collapseList[this.deleteunitindex].isShow = true;
					this.showChildel = false;
				})
