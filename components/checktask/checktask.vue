<template>
	<view class="checktask">
		...
		<view class="checktask-content" v-if="subIndex==0">
			...
			<scroll-view scroll-y style="height:80vh;width: 100%;" @scrolltolower="onreachBottom" v-else>
				<u-swipe-action v-for="(item,index) in list" :key="index" @open="open" :index="index" :options="item.options"
				 @click="click" :show="item.show" class="swipe" btn-width="120" @close="close">
					<view class="part" @click="jump(item.id,item.taskName,item.type)">
						<view class="top-part">
							<view class="left">
								<view class="name">
									{{item.taskName}}
									<u-badge type="error"  v-if="item.total_issue!=0" :count="item.total_issue" class="badge-msg"></u-badge>
								</view>
								<view class="detail">
									<view class="title">
										检查准则：
		...
		<view class="checktask-content" v-if="subIndex==1">
			<view class="empty" v-if="list.length == 0">
				<image src="../../static/loading.png" mode="" v-if="changing" class="changing"></image>
				<image src="../../static/nodata.png" mode="" v-else></image>
				<view class="empty-tips" v-if="changing">
...
<script>
	...
	export default {
		...
		onShow() {
			if (uni.getStorageSync("2")) {
				this.getlist(0)
				uni.setStorageSync("2", false);
			}
		},
		created() {
		...
		methods: {
			...
			click(index, index1) {
					...
					helper.request(helper.websiteUrl + 'v1/queryUserBySpecialty', param, 'POST', true, res => {
						...
						if (functionid == 3) {
							if(this.list[index].type==2){
								...
								helper.request(helper.websiteUrl + 'v1/queryCheckReportUser', param, 'POST', true, res => {
									arr.push({
										text: res.NAME,
										id: res.USER_ID
									})
				...
				if (this.list[index].options[index1].text == '指派') {
					...
					this.groupAllocationList(this.list[index].id)
					this.showAllot = true;
				}
			...
			sureproblem() {
				...
				helper.request(helper.websiteUrl + 'v1/groupAllocation', param, 'POST', true, res => {
					this.closeproblem();
					this.getlist(0)
				})
			},
			...
			//指派拉取当前用户得检查列表
			groupAllocationList(id){
				let param = {
					uid:uni.getStorageSync("USER_ID"),
					id:id
				}
				helper.request(helper.websiteUrl + 'v1/groupAllocationList', param, 'POST', true, res => {
					if(res.length!=0){
						let arr =[]
						res.forEach(item=>{
							let arr1 = []
							item.normDetail.forEach(val=>{
								arr1.push({
									detail: val.normDetailName,
									grade: '400',
									person: '',
									id:val.id
								})
