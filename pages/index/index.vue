<template>
	<view class="index">
		<a-head bgColor='#4abdb4' text="验收评价" :showList='true' :showBack="false" :msgdata="msgdata" :curMode="curMode"
		 :username="leftname" :phone='leftphone' ></a-head>
		...
		<view class="content" v-if="current=='/pages/checktask/checktask'">
			<!-- 这里是检查任务-->
			<checktask :offset='leadheight' :curMode='curMode' ref="checktask"></checktask>
		</view>
		<!--页面加载动画-->
...
<script>
	...
	export default {
		...
		if (uni.getStorageSync("flagtask")) {
			// uni.setStorageSync("flagtask", false);
			uni.removeStorageSync("flagtask")
			let taskIndex=3;
			if(uni.getStorageSync("taskIndex")){
				taskIndex=uni.getStorageSync("taskIndex")
				this.$refs.checktask.getlist(taskIndex);
			}else{
				this.$refs.checktask.getlist(3);
			}		   
		}
			const {
				windowWidth,
