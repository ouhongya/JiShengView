<template>
	<view class="login">
		...
		<a-login :showBack="false" bgColor='#4abdb4'></a-login>
		...
		<view class="content" :class="ipadW?'large':''">
			...
			<view class="name">
				...
				<u-input v-model="username" type="text" :border="false" placeholder="请输入登录账号" height="70" />
			</view>
			<view class="phone">
...
<script>
	...
	import Alogin from '@/components/Alogin/index.vue'
	export default {
		components: {
			'a-login': Alogin, 			
		},
		...
		created() {
			...
			if (windowWidth > 600 && ratio < 1.7&&windowHeight>600) {
				this.ipadW=true
			} else {
				this.ipadW=false
			}
			uni.setStorageSync('USER_ID', "13531231");
			helper.request(helper.websiteUrl + "v1/sendMessage", {
				'uid': '132135153'
			}, 'POST', false, (data) => {})
		},
		onLoad() {
