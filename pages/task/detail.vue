...
<script>
	...
	export default {
		...
		methods: {
			...
			checkboxChange(e, index, index1) {
				...
				if (!e.checked) {
					...
					helper.request(helper.websiteUrl + 'v1/cancelCheckingTask', param, 'POST', true, res => {
						this.queryTaskDetailRowCheck(this.list[index].tableList[0].detail_id, index, index1);
					})
				} else {
			...
			// }
			//算当前行的得分情况
			queryTaskDetailRowCheck(id,index,index1){
					  let param = {
						  uid:uni.getStorageSync('USER_ID'),
						  id:id
					  }
					helper.request(helper.websiteUrl + 'v1/queryTaskDetailRowCheck', param, 'POST', true, res => {
						this.list[index].gotgrade += res.score
						this.list[index].tableList[index1].gotscore = res.totalScoreRow
						this.list[index].lostgrade = res.points
						this.list[index].prograss = res.sleep
						this.list[index].tableList[index1].state=res.status
					})
			}
		}
	}
